        ...
        foreach ($regles as $regle => $parametre)
        {
            switch ($regle)
            {
                ...
                case 'plage_max':
                    ...
                    break;
                case 'mdp_robuste':
                    if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $valeur)) {
                        $this->messagesErreurs[] = "Le mot de passe doit comporter au moins 8 caractères, une lettre minuscule, une majuscule, un chiffre et un caractère spécial.";
                        $estValide = false;
                    }
                    break;
